import type { CSSProperties, ComponentType, SVGProps } from "react";
import * as Flags from "country-flag-icons/react/3x2";
import { hasFlag } from "country-flag-icons";

type BaseFlagProps = SVGProps<SVGSVGElement> & {
  title?: string;
};

type CountryFlagProps = BaseFlagProps & {
  code?: string | null;
  style?: CSSProperties;
};

function NepalFlag({ title, ...rest }: BaseFlagProps) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 513 342" {...rest}>
      {title && <title>{title}</title>}
      <path fill="#0052B4" d="M0 342V-.2L281.1 175H87.8l184 167z" />
      <path fill="#D80027" d="M243.4 164.8 9.2 19.3v313.5h235.2l-184.1-168z" />
      <path
        fill="#FFF"
        d="m62.4 204.1 7.9 16.2 14.9-10.1-1.3 18 18-1.3-10.1 14.9 16.2 7.9-16.2 7.8 10.1 15-18-1.3 1.3 17.9L70.3 279l-7.9 16.2-7.8-16.2-14.9 10.1 1.3-17.9-18 1.3 10.1-15-16.2-7.8 16.2-7.9L23 226.9l18 1.3-1.3-18 14.9 10.1zm-5.3-103.7 5.3 7.7 5.4-7.7 2 9.1 7.9-5-1.7 9.2 9.3-1.7-5.1 7.9 9.2 2-7.8 5.4 7.8 5.3-9.2 2 5.1 7.9-9.3-1.7 1.7 9.3-7.9-5.1-2 9.2-5.4-7.7-5.3 7.7-2-9.2-7.9 5.1 1.7-9.3-9.3 1.7 5.1-7.9-9.2-2 7.7-5.3-7.7-5.4 9.2-2-5.1-7.9 9.3 1.7-1.7-9.2 7.9 5z"
      />
      <path
        fill="#FFF"
        d="M18.5 108.7s4.8 27.7 44 27.7 44-27.7 44-27.7c0 9.4-6 46.5-44 46.5s-44-38.2-44-46.5z"
      />
    </svg>
  );
}

export function CountryFlag({ code, ...svgProps }: CountryFlagProps) {
  const normalizedCode = code?.trim().toUpperCase();
  if (!normalizedCode) {
    return null;
  }

  if (normalizedCode === "NP") {
    return <NepalFlag {...svgProps} />;
  }

  if (!hasFlag(normalizedCode)) {
    return null;
  }

  const FlagIcon = (Flags as Record<string, ComponentType<SVGProps<SVGSVGElement>>>)[normalizedCode];
  if (!FlagIcon) {
    return null;
  }

  return <FlagIcon {...svgProps} />;
}
